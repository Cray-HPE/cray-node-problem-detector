...
lint:
	CMD="lint charts/cray-node-problem-detector" $(MAKE) helm

dep-up:
	CMD="dep up charts/cray-node-problem-detector" $(MAKE) helm
...
ifdef CHART_VERSIONS
	CMD="package charts/cray-node-problem-detector --version $(word 1, $(CHART_VERSIONS)) -d packages" $(MAKE) helm
else
...
images:
	CHART=charts/cray-node-problem-detector $(MAKE) -s extracted-images annotated-images | sort -u
...
clean:
	$(RM) -r .helm packages charts/cray-node-problem-detector/charts
